...
const Edit = () => {
    ...
    const loadPurchaseInfo = () => {
        axios.get(`v1/purchases/get/${params.id}`, {
            ...
            if(status === 200){
                ...
                setPurchaseProducts(data.purchaseProducts.map(({productId, unitVariationId, qty, bonusQty, tradePrice, purchasePrice, vat, productName, unitVariation}) => {
                    return {
                        productId: productId,
    ...
    const onFormSubmitHandler = (event) => {
       ...
       data.purchaseProducts = purchaseProducts;
        ...
        axios.post(`/v1/purchases/${params.id}/edit`, data, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        }).then(({ status }) =>
        {
            if (status === 201) {
                toast.success('purchases has been updated.');
                dispatch(closeLoader());
            }
