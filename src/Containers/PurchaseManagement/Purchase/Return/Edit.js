...
const Create = () => {
    ...
    const loadReturnInvoice = () => {
        axios.get(`v1/purchase-returns/get/${params.id}`, {
            headers: {
            ...
            if(status === 200){
                ...
                updatedControls.supplier.value = data.supplierName;
                updatedControls.store.value = data.storeName;
                updatedControls.invoiceNumber.value = data.purchaseInvoiceNumber;
                ...
                setReturnProducts(data.products.map(({purchaseProductId, qty, tradePrice, vat, product, unitVariation}) => {
                    return {
                ...
                setLoading(false);
                loadPurchaseProducts(data.purchaseId);
            }
        }).catch(error => {
    ...
    const loadPurchaseProducts = (purchaseId) => {
        dispatch(openLoader());
        axios.get(`v1/purchases/get/${purchaseId}/purchase-products`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        }).then(({status, data}) => {
            if(status === 200){
                setPurchaseProducts(data.map(({id, qty, purchasePrice, vat, productName, unitVariation}) => {
                    return {
    ...
    const onFormSubmitHandler = (event) => {
        ...
        axios.post(`/v1/purchase-returns/${params.id}/edit`, data, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        }).then(({ status }) =>
