...
const Index = () => {
    ...
    useEffect(() => {
        loadHeads(pageNumber);
    }, [])

    const loadHeads = (page) => {
        const searchParams = new URLSearchParams();
        searchParams.set("page", page.toString());
        searchParams.set('size', PAGE_SIZE.toString());

        axios.get(`/v1/accounting/heads?${searchParams.toString()}`, {
            headers: {
    ...
    const deleteHandler = () => {
        ...
        dispatch(openLoader());
        axios.post(`/v1/accounting/heads/${selectedId}/delete`, null, {
            headers: {'Authorization' : `Bearer ${token}`}
        }).then(({status}) => {
            if(status === 204){
                toast.success('Head has been deleted success.')
                loadHeads(pageNumber);
    ...
    const {rows, totalRows} = heads;
    return (
        <MainSection title='Heads' breadcrumbPaths={[{name: 'Home', path: '/'}, {name: 'Dashboard', path: '/accounting'}]}>
            <div className='row'>
                    ...
                    {rows.length === 0 ? (
                        ...
                        <div className='table-responsive'>
                            <table className="table table-sm table-bordered">
                                ...
                                <tbody>
                                {rows.map(({id, name, descriptions}, idx) => (
                                    <tr key={id}>
            ...
            <AlertDialog
                alertMessage={'Are you sure want to delete head?'}
                open={alertDialogue}
