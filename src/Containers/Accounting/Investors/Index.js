...
const Index = () => {
    ...
    useEffect(() => {
        loadInvestors(pageNumber);
    }, [])

    const loadInvestors = (page) => {
        const searchParams = new URLSearchParams();
        searchParams.set("page", page.toString());
        searchParams.set('size', PAGE_SIZE.toString());

        axios.get(`/v1/accounting/investors/gets?${searchParams.toString()}`, {
            headers: {
    ...
    const deleteHandler = () => {
        ...
        dispatch(openLoader());
        axios.post(`/v1/accounting/investors/${selectedId}/delete`, null, {
            headers: {'Authorization' : `Bearer ${token}`}
        }).then(({status}) => {
            if(status === 204){
                toast.success('Investor has been deleted success.')
                loadInvestors(pageNumber);
    ...
    const {rows, totalRows} = investors;
    return (
        <MainSection title='Investors' breadcrumbPaths={[{name: 'Home', path: '/'}, {name: 'Dashboard', path: '/accounting'}]}>
            <div className='row'>
                    ...
                    {rows.length === 0 ? (
                        ...
                        <div className='table-responsive'>
                            <table className="table table-sm table-bordered">
                                ...
                                <tbody>
                                {rows.map(({id, name, email, phoneNumber, descriptions}, idx) => (
                                    <tr key={id}>
                                        ...
                                        <td className='text-center'>
                                            <Dropdown
                                                ...
                                                menuItems={[
                                                    {name: 'Edit', onClicked: () => navigate(`/accounting/investor/update/${id}`)},
            ...
            <AlertDialog
                alertMessage={'Are you sure want to delete Investor?'}
                open={alertDialogue}
